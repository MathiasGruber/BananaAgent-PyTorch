...
class BaseDQN(nn.Module):
    """Base model with some convenience functions used both for DQN and DuelDQN"""
...
class DuelDQN(BaseDQN):
    ...
    def __init__(self, 
        ...
        if state_type == 'discrete':
            ...
            # Value function (V)
            self.fc_v = nn.Linear(dense_layers[0], dense_layers[1])
            self.out_v = nn.Linear(dense_layers[1], 1)

            # Advantage function (A)
            self.fc_a = nn.Linear(dense_layers[0], dense_layers[1])
            self.out_a = nn.Linear(dense_layers[1], action_size)
            ...
            # Value function (V)
            self.fc_v = nn.Linear(512, dense_layers[1])
            self.out_v = nn.Linear(dense_layers[1], 1)

            # Advantage function (A)
            self.fc_a = nn.Linear(512, dense_layers[1])
            self.out_a = nn.Linear(dense_layers[1], action_size)
